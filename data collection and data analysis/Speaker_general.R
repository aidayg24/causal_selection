...
agg_over_valence <- dat_all %>%
  group_by(structure) %>%
  summarise(pct_normal = mean(normal_choice, na.rm = TRUE) * 100,
            n = n(), .groups = "drop")
...
p2_observed_agg <- ggplot(agg_over_valence, aes(x = structure, y = pct_normal)) +
  ...
  labs(title = "Observed (aggregated over valence framing)",
       x = "Causal structure", y = "Percentage choosing the normal cause") +
  theme_minimal()
...
fmt_y <- label_percent(accuracy = 1, scale = 1)

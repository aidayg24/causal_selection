...
# ===== Descriptive summaries with bootstrapped 95% CIs =====
...
# (1) Expected from prior literature — no error bars
expected <- tibble(
  structure  = factor(c("disjunctive","conjunctive"), levels = c("disjunctive","conjunctive")),
  pct_normal = c(68, 12)
)

pL_prior <- ggplot(expected, aes(x = structure, y = pct_normal, fill = structure)) +
  ...
  labs(
    ...
    subtitle = "Listener task — average normal-cause inference rates",
    x = "Causal structure",
    y = "Percentage inferring a normal cause"
  ) +
  theme_minimal() +
  theme(legend.position = "none")

# (2) Observed (aggregated over valence)
pL_agg <- ggplot(agg_over_valence, aes(x = structure, y = pct_normal)) +
  ...
  scale_y_continuous(labels = fmt_y, limits = c(0,100),
                     expand = expansion(mult = c(0, .1))) +
  labs(
    title = "Listener (aggregated over valence framing)",
    x = "Causal structure",
    y = "Percentage inferring a normal cause"
  ) +
  theme_minimal()
...
pL_pos <- by_valence %>%
  ...
  scale_y_continuous(labels = fmt_y, limits = c(0,100),
                     expand = expansion(mult = c(0, .1))) +
  labs(
    title = "Listener: positive framing",
    x = "Causal structure",
    y = "Percentage inferring a normal cause"
  ) +
  theme_minimal()
...
pL_neg <- by_valence %>%
  ...
  scale_y_continuous(labels = fmt_y, limits = c(0,100),
                     expand = expansion(mult = c(0, .1))) +
  labs(
    title = "Listener: negative framing",
    x = "Causal structure",
    y = "Percentage inferring a normal cause"
  ) +
  theme_minimal()

# (5) Faceted by valence
pL_facets <- ggplot(by_valence, aes(x = structure, y = pct_normal, fill = valence)) +
  geom_col(width = 0.6) +
  geom_errorbar(aes(ymin = ci_low, ymax = ci_high), width = 0.2, linewidth = 0.6) +
  ...
  labs(
    title = "Listener: normal-cause inference by structure",
...
ggsave("images/listener's plots/figL-expected-prior.png", pL_prior, width = 6, height = 4, dpi = 300)

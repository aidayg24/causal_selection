...
dat <- read_csv(
  "results_35_Causal_selection_V3_final_conj_AR.csv",
...
listener_long <- listener_long %>%
  mutate(
    ...
    question_id    = factor(question_id, levels = c("instrument","game","study","superpower"))
  )
...
listener_long %>% count(question_id, cause_inferred) %>% adorn_totals("row")
listener_long %>% count(valence, cause_inferred)
listener_long %>% distinct(submission_id) %>% nrow()
...
ggsave("listener_valence_plot_conj.png", width = 6, height = 4)
...
get_prior(m_listener_ord)
formula(m_listener_ord)

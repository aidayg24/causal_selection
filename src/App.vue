<template>
  <Experiment title="Experiment">
    ...
    <Screen :title="'Warm-up'">
      <ForcedChoiceScreen
        :question="'In the previous question, what was the winning rule for Alice?'"
        :options="['Alice needs exactly 2 points to win.', 'Alice needs at least 1 point to win.']"
        :feedbackTime="-1"
      >
        <template #feedback>

          <!-- Correct if response matches the condition-dependent answer -->
          <p
            v-if="
              $magpie.measurements.response ===
              (condition === 'conjunctive'
                ? 'Alice needs exactly 2 points to win.'
                : 'Alice needs at least 1 point to win.')
            "
            class="feedback correct"
          >
            Correct!
          </p>
          <p v-else class="feedback incorrect">Incorrect!</p>
    ...
    <template v-for="(t, i) in Ex1TrialsShuffled" >
      <Screen :key="`ex1-${i}`" :progress="i/totalTrials">
        ...
        <ForcedChoiceScreen
          ...
          :response.sync= "$magpie.measurements[condition+t.tag]" />
...
export default {
  ...
  data() {
    ...
    // const condition = _.shuffle(["conjunctive", "disjunctive"])[0];
    const condition = "disjunctive";
  ...
  computed: {
    totalTrials() {
      return this.Ex1TrialsShuffled.length + this.Ex2TrialsShuffled.length; 
    },
    ex1Len() {
